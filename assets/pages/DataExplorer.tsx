...
export function DataExplorer() {
    ...
    const TabGroup = ({ title, tabs }: { title: string, tabs: Record<string, string> }) => (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            <span style={{ fontSize: '11px', fontWeight: 700, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.05em', paddingLeft: '4px' }}>{title}</span>
            <div style={{ display: 'flex', gap: '4px', backgroundColor: '#f3f4f6', padding: '4px', borderRadius: '12px' }}>
        ...
        <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
                <div>
                    ...
                    <p style={{ margin: '4px 0 0', color: '#6b7280' }}>Inspect physical storage across the hybrid architecture.</p>
                </div>
                <button 
                                                ...
                                                <td key={j} style={{ padding: '16px 24px', verticalAlign: 'top', color: '#4b5563' }}>
                                                    {typeof value === 'object' ? (
                                                        ...
                                                        <span style={{ 
                                                            ...
                                                            fontSize: key.toLowerCase().includes('id') ? '12px' : '13px'
                                                        }}>
                                                            {String(value)}
                                                        </span>
                                                    )}
